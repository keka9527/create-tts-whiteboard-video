...
默认动画方式也升级为“逐主体描画”：先完整画完一个人物、动物、设备或图标组的线稿，再完成这个主体的上色，然后手部移动到下一个主体。它不再把整张图当成扫描幕从左到右擦出，也不会用等宽切条把对象从中间切开。
...
- 语义主体标注：每个主体完整描线、上色后再画下一个
- 手持画笔跟随当前真实笔触，只在落笔阶段显示
- `exclusive-owner-v2` 唯一遮罩归属，防止矩形死区和缺图
- 保留可选 `spatial-scan` 整图扫描模式
...
- `semantic`（默认）：按完整语义主体逐个描线和上色，适合人物、动物、设备、图标组等叙事画面。
- `spatial-scan`（可选）：整张画面从左到右连续推进，只在用户明确需要扫描幕效果时使用。

切换为整图扫描模式：

```powershell
python scripts/render_project.py <其他参数> --reveal-order spatial-scan
```

默认逐主体模式无需额外参数，也可以显式传入 `--reveal-order semantic`。
